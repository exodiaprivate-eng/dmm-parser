...
| Reflection (PA-side: `.prefab`, `.parg`, `.pasg`, `.paseqc`, `.paa_metabin`) | catalogued | 🟡 | **231 distinct PA reflection classes / 938 canonical fields** harvested via pycrimson (corrected 2026-05-10 iter 22 — earlier 8,362 number conflated classes with the 8,131 prefab-path instance dumps). Documented in `ENGINE_INTERNALS.md` master class index. **Not natively parsed** in dmm-parser — pycrimson is an external tool; native Rust deserializers would be Layer-A future work. |
