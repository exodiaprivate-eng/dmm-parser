...
```json
{
  ...
  "overlays": {
    ...
    "dmmgen": {
      ...
      "owner_version": "1.3.4",
      "mod_format": "v2",
      "content": "byte-replace mod (legacy)",
      "updated": "2026-05-01T10:00:00Z",
      "files": ["someinfo.pabgb"]
    }
  },
  "audit_log": [
    {
...
| `overlays.<slot>.owner` | string | yes | One of: `"DMM"`, `"CrimsonGameMods"`, `"JMM"`, `"Unknown"` |
| `overlays.<slot>.owner_version` | string | yes | Semver of the writing tool |
| `overlays.<slot>.mod_format` | string | yes | One of: `"v1"`, `"v2"`, `"legacy"`, `"v3"`, `"v3.1"` (see §2.5) |
...
| `overlays.<slot>.signature` | string | conditional | REQUIRED when `mod_format` ∈ {`v3`, `v3.1`}; OMITTED for `v1`/`v2`/`legacy` |
| `overlays.<slot>.key_id` | string | conditional | REQUIRED when `signature` is present; otherwise OMITTED |
| `audit_log` | array | optional | Append-only log of mount/unmount events |

### 2.4 Backward Compatibility — Reading v1 Protocol State Files

Authorized Tools implementing Protocol v2 MUST also be able to read Protocol v1 State Files (no `protocol_version` field, no `mod_format` field, no `signature` / `key_id`). When reading legacy v1 protocol entries:

  - Entries with known `owner` values (`DMM`, `CrimsonGameMods`, `JMM`) SHOULD be treated as **soft-trust** — accepted at face value, logged as "legacy v1 protocol entry"
  - Treat as if `mod_format: "legacy"` for purposes of conflict resolution
  - Entries with unknown `owner` SHOULD be treated as `"Unknown"` and ignored for ownership purposes

Writing a v2 entry into a v1 file upgrades the file to v2, but **does not** retroactively sign existing v1 entries — they remain legacy entries with their original semantics.

### 2.5 Mod Format Field — Signing Scope

The `mod_format` field declares which mod format the overlay was generated by. **Signing is REQUIRED for v3 and v3.1 mounts.** Legacy mod formats (v1, v2) continue to use unsigned entries indefinitely.

| Mod Format | Signing | Description |
|---|---|---|
| `"v1"` | NOT signed | First-generation byte-diff mods (legacy) |
| `"v2"` | NOT signed | Byte-replace mods (legacy, JMM/Swiss-Knife era) |
| `"legacy"` | NOT signed | Pre-Protocol v2 entries from old DMM/SWISS versions, OR entries with no `mod_format` field |
| `"v3"` | **REQUIRED** | Field JSON v3 single-target — Authorized Tool mounts MUST sign |
| `"v3.1"` | **REQUIRED** | Field JSON v3.1 multi-target — Authorized Tool mounts MUST sign |

**Rationale**: The signing layer protects the entire Field JSON ecosystem (v3 + v3.1) — the formats authored by RicePaddySoftware and used by DMM and SWISS to coordinate field-level mod application. v3 and v3.1 share the same parser dispatch, the same dmm-parser engine, and the same authorial origin; treating them as a single signing scope reflects how they actually function.

Legacy mod formats (v1, v2) predate the Field JSON design entirely and are intentionally exempt — they are simple byte-diff/byte-replace operations that don't traverse the field JSON pipeline. This ensures DMM continues to mount mods authored under older formats without breakage.

**Default for missing `mod_format` field**: An entry with no `mod_format` field (e.g., entries written by pre-Protocol-v2 DMM/SWISS versions, or v1 protocol files being read) is treated as `"legacy"` — soft-trust accepted, no signing required. This protects existing user installations during upgrade. New mounts by Protocol v2-aware tools always set `mod_format` explicitly.

**Future deprecation**: RicePaddySoftware reserves the right to deprecate `v1` and `v2` mod format support in a future release. When that happens, this Protocol will be amended to either remove those rows from this table or upgrade them to require signatures. Until that explicit amendment, legacy mod formats MUST continue to mount without protocol-level rejection.

**Conflict resolution applies regardless of format**: The conflict scan (§8) treats legacy and signed entries identically for the purpose of determining "who owns file X" — only the verification status differs. A v1 mount and a v3.1 mount can coexist in the State File simultaneously.
...
The verification algorithm depends on the entry's `mod_format`:

```
1. Read entry.mod_format (default "legacy" if not present).

2. If mod_format ∈ {"v1", "v2", "legacy"}:
   → Soft-trust path. No signature is REQUIRED for these formats.
   → If entry.signature is absent: ACCEPT as legacy (status = LegacyUnsigned).
   → If entry.signature is present: still verify per steps 3-9 below;
     a malformed/forged signature on a legacy entry SHOULD be reported
     but the entry MAY still be accepted with status = LegacyUnsigned to
     preserve mount functionality.

3. If mod_format ∈ {"v3", "v3.1"}:
   → Signing is REQUIRED. Continue to step 4.

4. Look up entry.key_id in the embedded TRUST_ROOTS table.
5. If not found OR key is outside its valid_from/valid_until window:
   → REJECT (status = UnknownKey, treat owner as "Unknown")
6. If trust_root.owner != entry.owner:
   → REJECT (status = OwnerMismatch — a key signed for a different owner)
7. Re-compute canonical_entry_bytes from entry (excluding signature/key_id).
8. Decode entry.signature from base64.
9. ed25519_verify(trust_root.public_key, canonical_entry_bytes, sig_bytes).
10. If verify fails → REJECT (status = InvalidSignature).
11. Otherwise → ACCEPT (status = Valid, entry is authentic).
```

**Critical**: `LegacyUnsigned` is a permanent acceptance state for v1/v2/legacy mod formats — it is NOT a transition-only state. Authorized Tools MUST continue to honor unsigned entries from these formats indefinitely (subject to future Protocol amendments).

`v3` and `v3.1` are the mod formats that REQUIRE verification. A v3 or v3.1 entry without a valid signature is rejected.

**Backward compatibility note**: Entries from pre-Protocol-v2 DMM/SWISS releases lack a `mod_format` field entirely. The default value `"legacy"` (per §2.5) routes them through the soft-trust path, preserving compatibility with existing user installations. Once a Protocol v2 tool re-mounts the same overlay, the new entry explicitly sets `mod_format: "v3"` or `"v3.1"` and signing applies from that point forward.
