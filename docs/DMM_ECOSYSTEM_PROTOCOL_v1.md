...
The `mod_format` field declares which mod format the overlay was generated by. **Signing is REQUIRED for v3 and v3.1 mounts only.** Legacy mod formats continue to use unsigned entries indefinitely.
...
| `"legacy"` | NOT signed | Pre-Protocol v2 entries from old DMM/SWISS versions |
| `"v3"` | NOT signed | Initial Field JSON v3 (single-target). Soft-trust accepted. |
| `"v3.1"` | **REQUIRED** | Field JSON v3.1 multi-target — Authorized Tool mounts MUST sign |

**Rationale**: The signing layer protects the v3.1 ecosystem — the format that ties DMM and SWISS together via Field JSON v3.1 spec compliance. Legacy mod formats predate the protocol and are intentionally exempt from signing requirements. This ensures DMM continues to mount mods authored under older formats without breakage.
...
2. If mod_format ∈ {"v1", "v2", "legacy", "v3"}:
...
**Critical**: `LegacyUnsigned` is a permanent acceptance state for v1/v2/legacy/v3 mod formats — it is NOT a transition-only state. Authorized Tools MUST continue to honor unsigned entries from these formats indefinitely (subject to future Protocol amendments).

`v3.1` is the only mod format that REQUIRES verification. A v3.1 entry without a valid signature is rejected.
