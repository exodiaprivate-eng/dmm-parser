...
| `.pathc` | ⚠️ (runtime lookup cache, not mod-relevant) | **Texture header collection** (2.3 MB). Class `TextureHeaderCollection` (IDA 0x144e4dd28). Created by `packTextureHeaderCollection` engine function — likely a HASH-MAP / runtime lookup cache for fast texture metadata access, not a named-field record format. No `_textureHash`/`_pixelFormat`/`_mipCount`/`_textureWidth` field names exist as canonical strings (searched IDA, all returned 0). For mod authors: textures are modded via `.dds` replacement, not via this cache (the engine rebuilds the cache from DDS headers at load time). Per-byte semantic decode would need IDA decompile of the runtime hash-map writer. Mostly safe to ignore. | none — round-trip via opaque body suffices |
...
| `.paproj` | ⚠️ (structure + 8 subclasses + 11 named fields identified) | **Projectile data** binary. Layout: `u32 record_count + u32 type_hash + u32 something + records`. **8 polymorphic CommonProjectile subclasses** (each with Client/Common/Server triplet variants): `CommonProjectile` (base), `_Repeat`, `_AnimatedSpline`, `_Wave`, `_RagdollCollision`, `_AttachToActor`, `_Laser`. Plus `CommonProjectileBuffProcessor` for buff application. **11 named fields per record** (iter 11): `_projectileKey`, `_projectileShotKey`, `_projectileShotCount`, `_projectileShotInterval`, `_projectileShotSpread`, `_projectileHitRate`, `_projectileHeightOffset`, `_projectileCreateDelayTime`, `_projectileChasePhysicsMaterialHash`, `_projectileKeyColor`, `_projectileInfoPackage`. Per-byte→per-name mapping needs IDA pseudocode of each subclass's reader. | none |
...
| `.pai` | ⚠️ (polymorphic AI chart, 20+ subclasses identified) | **AI behavior chart data** (3 MB). Class `AIChart` (IDA 0x144ca9630), enum `AIChartObjectType`. Records are POLYMORPHIC — **20+ AIActionChart subclasses** found in IDA: `_Normal`, `_DefaultAction`, `_Event`, `_SplinePathMoveAction`, `_NPCScheduleAction`, `_NPCScheduleIngEndAction`, `_NPCScheduleWaitAction`, `_NPCScheduleExtraAction`, `_Custom`, `_ActionPoint`, `_PatrolPointAction`, `_PatrolPointEndAction`, `_PitchJumpAction`, `_StageEndBehaviorAction`, `_StageEndBehaviorEndAction`, `_StageEndBehaviorDelayIdleAction`, `_SequencerEndInputAction`, `_SequencerAction`, `_CatchSpawnAction`, `_AISpawnAction` (per `CompressedObjectMemoryPool` template instantiations). For mod authors: this file controls NPC behavior patterns; per-subclass field decode would unlock AI behavior mods. Per-subclass byte-offset mapping needs IDA decompile of each subclass's reader. | none |
...
| `.questgaugecount` | ⚠️ (structure + class hierarchy identified) | **Quest gauge counter**. Layout `u32 count(=382) + 382 × QuestGaugeCountData records`. Per-record class hierarchy from IDA: `QuestGaugeCountData` (with `_stageList: CArray<QuestGaugeCountData_Stage>` field at IDA 0x144b11206) → nested `QuestGaugeCountData_Stage` (with `_stageType` field at IDA 0x144b1d125). The 0xFFFFFFFF markers separate stage boundaries. Per-byte decode needs IDA pseudocode of `QuestGaugeCountDataReader`. Only 1 file in install. | `parse_count_record_table_bytes` returns count + opaque body |
