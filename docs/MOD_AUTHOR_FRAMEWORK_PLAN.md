...
- [ ] **P1** — Find a real `.paloc` file on disk (probably under game install's data folder, search PAZ groups for `*.paloc`). Write `references/paloc.hexpat` covering: entries (u64 category + u32+bytes key + u32+bytes value), trailer (verify the 16-bytes-per-entry hypothesis), trailing u32 count. Run `plcli run -i <sample> -p references/paloc.hexpat -v -d` and iterate until output makes sense.

- [ ] **P2** — Create `dmm-parser/src/paloc/` with `mod.rs`, `entry.rs`, `container.rs`. Define `PalocEntry { category: u8, key: BString, value: BString }` and `PalocContainer { flags: u16, entries: Vec<PalocEntry> }`. Add `pub mod paloc;` to `lib.rs`. Stub the public functions.

- [ ] **P3** — Implement plain-format (flags=0x0000) parser: `PalocEntry::read_from`, `PalocEntry::write_to`, `PalocContainer::read_from`. Round-trip test on a small plain sample. Verify byte-perfect.
...
- [ ] **P9** — Docs: update `docs/archive-format.md` with paloc spec, `docs/api.md` with Python API, `README.MD` table count from 122 → 123. Update `docs/CUSTOM_ITEM_CREATOR_V3_1.md` §1 to mark paloc ✅ and §6 to confirm Option A is now viable.

- [ ] **P10** — DMM apply integration: in `dmm-api-test/src-tauri/src/iteminfo/field_json_v3.rs`, add `paloc.pamt` target dispatch. Verify `apply_v3_for_target("paloc.pamt", ...)` routes through dmm-parser. End-to-end test: v3.1 mod with `set_localization` intent → DMM applies → game shows custom name.
