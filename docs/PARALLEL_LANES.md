...
|---|---|---|---|---|
| **A** (this) | `dmm-parser/` | `main` | Now (loop active, no-push mode) | `interaction_info` 99.2%, `condition_info` 99.78%, `gimmick_info` 99.95%. 13 ConditionData tag recipes touched (12 reduced histogram + tag 54 best-effort u32 body). Anti-disasm plateau reached. **TriggerGamePlayEventHandlerData ✅ FULLY SHIPPED via `1fc44e8`**: all 8 cases typed (5 unit + 3 with bodies including the nested polymorphic ForceField with sub-cases 0-3/4/5/7/8); GimmickInfo wired with `trigger_event_handler_list: Option<CArray<OptionalTriggerGamePlayEventHandlerData>>`. **No Tier 1.5 sub-fields remain.** 308/308 tests pass. |
