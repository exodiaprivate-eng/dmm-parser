...
pabgh_blob_table! { pub struct RoyalSupplyInfo<'a> { key: u16, blob_field: data_blob, } }
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test] fn roundtrip() {
        ...
        for (i, (k, s, e)) in ranges.iter().enumerate() {
            let mut c = *s; items.push(RoyalSupplyInfo::read_with_size(&data, &mut c, e-s).unwrap_or_else(|er| panic!("e{} k=0x{:x}: {}", i, k, er)));
            assert_eq!(c, *e);
        }
