...
mod tests {
    ...
    const PABGB: &str = r"/mnt/c/temp/GIT/CrimsonDesertUpdates/pabgb/2026-4-24/factionrelationgroup.pabgb";
    const PABGH: &str = r"/mnt/c/temp/GIT/CrimsonDesertUpdates/pabgb/2026-4-24/factionrelationgroup.pabgh";
    ...
    fn json_roundtrip() {
        ...
        for (i, (key, start, end)) in ranges.iter().enumerate() {
            let mut c = *start;
            let item = FactionRelationGroupInfo::read_from(&data, &mut c).unwrap();
            assert_eq!(c, *end, "entry {} key=0x{:x}: under/over-read", i, key);
            let dict = item.to_json_dict();
            let mut from_typed = Vec::new();
            item.write_to(&mut from_typed).unwrap();
            let mut from_json = Vec::new();
            FactionRelationGroupInfo::write_from_json_dict(&mut from_json, &dict)
