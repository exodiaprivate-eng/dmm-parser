...
impl PackGroupBuilder {
    ...
    /// Add a file using a specific compression algorithm, overriding the
    /// builder's default. Some game files MUST stay uncompressed even when
    /// the rest of the group is LZ4 — e.g. `iteminfo.pabgh`, which the
    /// game's loader compares `compressed_size` against `uncompressed_size`
    /// and rejects when the former is larger (LZ4 of random-looking
    /// key/offset bytes typically expands rather than shrinks).
