...
// ── CBytes ──────────────────────────────────────────────────────────────────
// Like CString but reads raw bytes without requiring UTF-8 validity.
// Used where the game stores binary-encoded strings (sub_1410A9B70 pattern).
...
impl<'a> BinaryRead<'a> for CBytes<'a> {
    fn read_from(data: &'a [u8], offset: &mut usize) -> io::Result<Self> {
        let length = u32::read_from(data, offset)?;
        let len = length as usize;
        check_remaining(data, *offset, len)?;
        let bytes = &data[*offset..*offset + len];
        *offset += len;
        Ok(CBytes { length, data: bytes })
    }
}

impl BinaryWrite for CBytes<'_> {
    fn write_to(&self, w: &mut dyn Write) -> io::Result<()> {
        self.length.write_to(w)?;
...
impl<'a> BinaryReadTracked<'a> for CBytes<'a> {
    fn read_tracked(
        data: &'a [u8],
        offset: &mut usize,
        path: &mut String,
