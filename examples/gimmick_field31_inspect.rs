...
//! Inspect bytes at field_31 failure point.
...
fn main() {
    ...
    for (key, start, end) in &ranges {
        ...
        if let GimmickTail::Decoded {
            field_30_u32_list, field_31_u32_list, post_blob, ..
        } = &item.tail {
            if field_30_u32_list.is_some() && field_31_u32_list.is_none() && !post_blob.is_empty() {
                if post_blob.len() >= 4 {
    ...
    println!("\nFirst-u32 distribution at field_31 failure point:");
