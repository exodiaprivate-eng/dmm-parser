...
use dmm_parser::binary::paschedulepath::{PaschedulePathFileSafe, TypedPaschedulePathFile};
...
fn main() {
    ...
    for (gi, group_name) in groups.iter().enumerate() {
        ...
        for dir in &pamt.directories {
            for f in &dir.files {
                if !f.name.to_ascii_lowercase().ends_with(".paschedulepath") { continue; }
                group_count += 1;
                ...
                match TypedPaschedulePathFile::parse(&bytes) {
                    Ok(typed) => match typed.to_bytes() {
                        Ok(written) => {
                            if written == bytes { stats.typed_byte_perfect += 1; }
                            else {
                ...
                match PaschedulePathFileSafe::parse(&bytes) {
                    Ok(safe) => match safe.to_bytes() {
                        Ok(rt) => if rt == bytes {
                            match safe {
                                PaschedulePathFileSafe::Decoded(_) => stats.safe_decoded_byte_perfect += 1,
                                PaschedulePathFileSafe::Raw { .. } => stats.safe_raw_byte_perfect += 1,
                            }
        ...
        if group_count > 0 {
            println!("[{:2}/{}] {}: {} paschedulepath files", gi + 1, groups.len(), group_name, group_count);
        }
    }
