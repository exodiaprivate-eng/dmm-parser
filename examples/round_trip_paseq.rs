...
//! Same shape as round_trip_pastage but for .paseq. Walks every vanilla
//! group, runs Tier 1.5 tokenizer parse + serialize, reports totals and
//! the first N failing files with byte deltas.

use dmm_parser::binary::pamt::{Compression, PackMeta};
use dmm_parser::binary::paseq::{PaseqFile, Token};
...
fn main() {
    ...
    for g in &groups {
        ...
        for d in &meta.directories {
            for f in &d.files {
                if !f.name.to_lowercase().ends_with(".paseq") { continue; }
                ...
                let parsed = match PaseqFile::parse(&body) {
                    Ok(p) => p,
                    Err(e) => {
                        fail += 1;
                ...
                if written == body {
                    ...
                    if json_total < 50 {
                        ...
                        match PaseqFile::from_json(&json) {
    ...
    println!("\n=== Summary ===");
    println!("Total .paseq files:   {}", total);
