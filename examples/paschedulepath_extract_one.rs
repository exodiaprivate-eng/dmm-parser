...
//! Extract a few `.paschedulepath` files for hex inspection.
...
const OUT_DIR: &str = r"C:\Users\corin\Desktop\CD DUMPING TOOLS\dmm-parser\target\paschedulepath_samples";
        ...
        for dir in &pamt.directories {
            for f in &dir.files {
                ...
                for (i, chunk) in bytes[..limit].chunks(16).enumerate() {
                    ...
                    h.push_str(" ");
                    for b in chunk { h.push(if (0x20..=0x7e).contains(b) { *b as char } else { '.' }); }
                    h.push('\n');
                }
