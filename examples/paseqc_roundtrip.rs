...
//! Walk every PAMT in the game install, extract every `.paseqc` file,
//! run it through `TypedPaseqcFile::parse` + `to_bytes`, and report
//! how many round-trip byte-perfect.

use dmm_parser::binary::pamt::PackMeta;
use dmm_parser::binary::paseqc::{PaseqcFileSafe, TypedPaseqcFile};
    ...
    for (gi, group_name) in groups.iter().enumerate() {
        ...
        for dir in &pamt.directories {
            for f in &dir.files {
                ...
                match PaseqcFileSafe::parse(&bytes) {
                    Ok(safe) => match safe.to_bytes() {
                        Ok(rt) => if rt == bytes {
                            match safe {
                                PaseqcFileSafe::Decoded(_) => stats.safe_decoded_byte_perfect += 1,
                                PaseqcFileSafe::Raw { .. } => stats.safe_raw_byte_perfect += 1,
                            }
        ...
        if group_count > 0 {
            println!("[{:2}/{}] {}: {} paseqc files", gi + 1, groups.len(), group_name, group_count);
        }
    }
