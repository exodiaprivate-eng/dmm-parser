...
//! Bulk BNK round-trip: parse + serialize every vanilla .bnk byte-exact.

use dmm_parser::binary::bnk::BnkFile;
    ...
    for g in &groups {
        ...
        for d in &meta.directories {
            for f in &d.files {
                ...
                let parsed = match BnkFile::parse(stored) {
                    Ok(p) => p,
                    Err(e) => {
                        fail_parse += 1;
                ...
                let written = match parsed.to_bytes() {
                    Ok(b) => b,
                    Err(e) => { fail_roundtrip += 1; println!("WRITE-ERR {}/{}: {}", d.path, f.name, e); continue; }
                };
                if written == stored {
                    pass += 1;
                    ...
                    if json_total < 50 {
                        json_total += 1;
                        let j = parsed.to_json();
                        if let Ok(rb) = BnkFile::from_json(&j) {
                            if let Ok(b) = rb.to_bytes() { if b == stored { json_pass += 1; } }
                        }
                    }
                } else {
                    fail_roundtrip += 1;
