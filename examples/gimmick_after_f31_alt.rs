...
//! Inspect bytes at field_32 failure point for entries that used f31_alt.
...
fn main() {
    ...
    for (key, start, end) in &ranges {
        ...
        if let GimmickTail::Decoded {
            f31_alt_001, field_32_u32_list, post_blob, ..
        } = &item.tail {
            // Entries that used f31_alt successfully but failed at field_32
            if f31_alt_001.is_some() && field_32_u32_list.is_none() && !post_blob.is_empty() {
                if post_blob.len() >= 4 {
    ...
    println!("\nFirst-u32 distribution after f31_alt at field_32 failure point:");
