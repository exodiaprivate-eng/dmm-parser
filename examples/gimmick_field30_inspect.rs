...
//! Inspect bytes at field_30+ failure point for the 1039 entries.
...
fn main() {
    ...
    for (key, start, end) in &ranges {
        ...
        if let GimmickTail::Decoded {
            field_29_u32_list, field_30_u32_list, post_blob, ..
        } = &item.tail {
            // Entries that typed field_29 but failed before field_30
            if field_29_u32_list.is_some() && field_30_u32_list.is_none() && !post_blob.is_empty() {
                if post_blob.len() >= 4 {
                    ...
                    *counts.entry(count).or_insert(0) += 1;
                    if shown < 5 {
                        ...
                        let preview = 96.min(post_blob.len());
                        for i in 0..preview {
    ...
    println!("\nFirst-u32 distribution at field_30 failure point:");
