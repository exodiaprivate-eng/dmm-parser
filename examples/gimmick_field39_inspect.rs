...
//! Inspect bytes at field_39 failure point (entries with field_38 typed but field_39 not).
...
fn main() {
    ...
    for (key, start, end) in &ranges {
        ...
        if let GimmickTail::Decoded {
            field_38_u32, field_39_u32_list, post_blob, ..
        } = &item.tail {
            if field_38_u32.is_some() && field_39_u32_list.is_none() && !post_blob.is_empty() {
                if post_blob.len() >= 4 {
    ...
    println!("\nFirst-u32 distribution at field_39 failure point:");
