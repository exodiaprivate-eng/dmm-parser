...
//! Count how many entries successfully use f31_alt or f39_alt chain.
...
fn main() {
    ...
    let mut f31_active = 0usize;
    let mut f31_full = 0usize; // typed all 64
    let mut f39_active = 0usize;
    let mut f39_full = 0usize;
    for (_key, start, end) in &ranges {
        ...
        if let GimmickTail::Decoded {
            f31_alt_001, f31_alt_064,
            f39_alt_001, f39_alt_064, ..
        } = &item.tail {
    ...
    println!("f31_alt_001 typed: {} (full chain {}/64 = {})", f31_active, f31_full, f31_full);
    println!("f39_alt_001 typed: {} (full chain {}/64 = {})", f39_active, f39_full, f39_full);
}
