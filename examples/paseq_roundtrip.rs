...
//! Walk every PAMT in the game install, extract every `.paseq` file,
//! run it through `TypedPaseqFile::parse` + `to_bytes`, and report
//! how many round-trip byte-perfect.

use dmm_parser::binary::pamt::PackMeta;
use dmm_parser::binary::paseq::{PaseqFileSafe, TypedPaseqFile};
    ...
    for (gi, group_name) in groups.iter().enumerate() {
        ...
        for dir in &pamt.directories {
            for f in &dir.files {
                ...
                match PaseqFileSafe::parse(&bytes) {
                    Ok(safe) => match safe.to_bytes() {
                        Ok(rt) => {
                            if rt == bytes {
                                match safe {
                                    PaseqFileSafe::Decoded(_) => stats.safe_decoded_byte_perfect += 1,
                                    PaseqFileSafe::Raw { .. } => stats.safe_raw_byte_perfect += 1,
                                }
                            } else {
        ...
        if group_count > 0 {
            println!("[{:2}/{}] {}: {} paseq files",
                gi + 1, groups.len(), group_name, group_count);
        }
    }
