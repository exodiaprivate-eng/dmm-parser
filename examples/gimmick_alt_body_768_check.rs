...
//! Check alt_body saturation at 768.
...
fn main() {
    ...
    let mut alt_body_768_with_residual = 0;
    let mut alt_body_768_residual_bytes = 0usize;
    ...
    for (_key, start, end) in &ranges {
        ...
        if let GimmickTail::Decoded {
            alt_body_640: a640, alt_body_768: a768,
            ...
            if a768.is_some() {
                ...
                if !post_blob.is_empty() {
                    alt_body_768_with_residual += 1;
                    alt_body_768_residual_bytes += post_blob.len();
                }
            }
    ...
    println!("alt_body chain after extension to 768:");
    println!("  alt_body_640 typed: {}", alt_body_640);
    println!("  alt_body_768 typed (full): {}", alt_body_768);
    println!("  alt_body_768 with non-empty post_blob: {} ({} bytes)", alt_body_768_with_residual, alt_body_768_residual_bytes);
